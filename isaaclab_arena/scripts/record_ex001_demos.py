...
"""Unified demo recording for EX001Arm — keyboard, VR, or remote WebSocket.

Teleop devices:
  - ``keyboard``                  — local keyboard control
  - ``ex001arm_openxr_bimanual``  — VR hand-tracking via OpenXR
  - ``remote``                    — physical arm via WebSocket (ros1_ws_bridge)

Usage::

    # Keyboard
    python -m isaaclab_arena.scripts.record_ex001_demos \\
        --enable_cameras ex001arm_cvpr_scene_put_blocks_to_color \\
        --teleop_device keyboard

    # Remote EE mode
    python -m isaaclab_arena.scripts.record_ex001_demos \\
        --teleop_device remote --remote_ip 10.100.21.249 \\
        ex001arm_cvpr_scene_put_blocks_to_color

    # Remote Joint mode
    python -m isaaclab_arena.scripts.record_ex001_demos \\
        --teleop_device remote --control_mode joint --remote_ip 10.100.21.249 \\
        ex001arm_cvpr_scene_put_blocks_to_color
"""
...
def _teleop_device_requires_xr(name: str | None) -> bool:
    if not name:
        return False
    lo = name.lower()
    return lo in _VR_DEVICE_NAMES or any(t in lo for t in ("handtracking", "openxr"))
...
parser.add_argument(
    ...
    help="Output path or directory for HDF5 demos. Defaults to ./data/<env_name>",
)
parser.add_argument(
    "--reset_duration", type=float, default=10.0,
    help="Duration of auto-reset trajectory in seconds. Default: 10.0",
)
parser.add_argument(
    "--teleop_device", type=str, default="keyboard",
    help="Teleop device: 'keyboard', 'ex001arm_openxr_bimanual', or 'remote'",
)
parser.add_argument(
    "--debug", action="store_true",
    help="Print diagnostic information (joint states, gripper values, etc.)",
)

# ── Remote-specific args ──────────────────────────────────────────────────────
parser.add_argument("--remote_ip", type=str, default="192.168.1.100",
                    help="WebSocket IP of robot PC (remote mode only)")
parser.add_argument("--remote_port", type=int, default=5555,
                    help="WebSocket port (remote mode only)")
parser.add_argument("--pos_scale", type=float, default=1.0,
                    help="Position delta scale (remote ee mode)")
parser.add_argument("--rot_scale", type=float, default=1.0,
                    help="Rotation delta scale (remote ee mode)")
parser.add_argument("--control_mode", type=str, default="ee", choices=["ee", "joint"],
                    help="Remote control mode: 'ee' (IK) or 'joint' (direct)")
parser.add_argument("--joint_signs", type=str, default="1,1,-1,-1,-1,1",
                    help="Per-joint sign multipliers, 6 comma-separated (remote joint mode)")
parser.add_argument("--joint_offsets", type=str, default="0,0,0,0,0,0",
                    help="Per-joint offsets in rad, 6 comma-separated (remote joint mode)")
...
def _parse_float_list(s: str, expected_len: int, name: str) -> tuple[float, ...]:
    ...
    if len(parts) != expected_len:
        raise ValueError(f"--{name} expects {expected_len} values, got {len(parts)}: '{s}'")
...
def _dump_joint_info(env) -> None:
    ...
    print("\n" + "=" * 80)
    print("  SIMULATION JOINT DIAGNOSTIC")
    print("=" * 80)
    ...
    if hasattr(env, "action_manager"):
        ...
        for term_name, term in env.action_manager._terms.items():
            jids = getattr(term, "_joint_ids", None) or getattr(term, "joint_ids", None)
            if jids is not None:
...
def create_teleop_interface(env, env_cfg):
    device_name = args_cli.teleop_device or "keyboard"

    if device_name == "remote":
        from isaaclab_arena.teleop_devices.ex001arm_ws_remote import (
            Ex001ArmWsRemoteCfg, Ex001ArmWsRemoteTeleop,
        )
        ...
        print(f"[INFO] Using remote WebSocket teleop ({mode_label}): "
    ...
    elif _teleop_device_requires_xr(device_name):
        from isaaclab.devices.teleop_device_factory import create_teleop_device
        teleop_callbacks = {}
        if hasattr(env_cfg, "teleop_devices") and device_name in env_cfg.teleop_devices.devices:
            teleop = create_teleop_device(device_name, env_cfg.teleop_devices.devices, teleop_callbacks)
        ...
        from isaaclab_arena.scripts.teleop_bimanual_keyboard import (
            BimanualSe3Keyboard, BimanualSe3KeyboardCfg,
        )
        cfg = BimanualSe3KeyboardCfg(sim_device=env.device)
        teleop = BimanualSe3Keyboard(cfg)
...
def main() -> None:
    ...
    if is_remote:
        if args_cli.control_mode == "joint":
            ...
            print("[INFO] Using EX001ArmPhysicalTeleopActionsCfg (IK 1:1)")
    ...
    # Recorder config — with or without camera images
    ...
    # Close startup placeholder file (RecorderManager creates it eagerly)
    if (
    ...
    running_recording = not is_vr  # VR waits for START; keyboard/remote start immediately
    ...
    def export_and_prepare_next():
        ...
        if is_vr:
            print("[INFO] Click START in VR to begin recording")
        print("=" * 60)
    ...
    rate_limiter = RateLimiter(DEFAULT_STEP_HZ) if (not is_vr and not is_remote) else None
    ...
    print(f"[INFO] Teleop device    : {device_name}")
    if is_remote:
    ...
    if is_vr:
        print("[INFO] VR Mode: Click START to begin, STOP to pause, RESET to discard")
